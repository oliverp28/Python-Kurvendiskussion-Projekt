""" Import of class linear_function
    Import of class quadratic_function
"""
...
class Input_Function:
    ...
    def __init__(self):
        ...
        while validation_bool is False:
            function = self.get_function()
            if self.validate_input(function) is True:  # Validate the input function
                if (
                    self.which_function(function) is True
                ):  # Determine the type of function
                    print("\n")
                    again_bool = ""
                    while again_bool != "ja" and again_bool != "nein":
                        again_bool = input(
                            "Willst du eine weitere Funktion eingeben ? (ja / nein):  "
                        ).lower()
                    if again_bool == "nein":
    ...
    def validate_input(self, function):
        ...
        for char in function:
            if char not in allowed_characters:
                print(
                    "Ungültige Eingabe. Bitte geben Sie eine gültige Funktion an.     ||      gültige Zeichen: '1234567890^*+- xX,.'"
                )
                return False
    ...
    def which_function(self, function):
        ...
        if "x^" in function or "x**" in function:
            if "x^2" in function:
                check_index = function.index("^") + 2
                if (check_index + 1) <= len(function) and function[
                    check_index
                ].isdigit():
                    print(
                        "Ungültige Eingabe. Bitte geben Sie eine gültige Funktion an."
                    )
                else:
                    # If the input contains "x^2", it's a quadratic function
                    self.validate_quadratic_function(function)
                    quadratic_function.Quadratic_Func(
                        self.split_function_quad(function=function.lower())
                    )  # Creates a Quadr_Func instance with lowercase x
                    return True
            elif "x**2" in function:
                check_index = function.index("*") + 3
                if (check_index + 1) <= len(function) and function[
                    check_index
                ].isdigit():
                    print(
    ...
    def validate_linear_function(self, function):
        ...
        if function.count("x") > 1:
            print(
                "Ungültige Eingabe. Lineare Funktionen sollten nur einen Term mit 'x' enthalten."
            )
    ...
    def validate_quadratic_function(self, function):
        ...
        if function.count("x^2") > 1 or function.count("x**2") > 1:
            print(
                "Ungültige Eingabe. Quadratische Funktionen sollten nur einen Term mit 'x^2' enthalten."
            )

        if function.count("x") > 2:
            print(
                "Ungültige Eingabe. Quadratische Funktionen sollten höchstens zwei Terme mit 'x' enthalten."
            )
    ...
    def split_function_quad(self, function):
        ...
        if (
            "-" in function
            and function.index("-") < function.index("x")
            ...
            "-" in function
            and function.index("-") < function.index("x")
            and function.count("-") == 2
        ):
            function = function.replace(" ", "")

            first_dash_index = function.find("-")  # index of the first "-"
            second_dash_index = function.find(
                "-", first_dash_index + 1
            )  # index of the second "-"

            function = (
                function[:second_dash_index] + ",-" + function[second_dash_index + 1 :]
            )

        elif (
            "-" in function
            and function.index("-") < function.index("x")
            and function.count("-") == 3
        ):
            function = function.replace(" ", "")
            ...
            third_dash_index = function.find(
                "-", second_dash_index + 1
            )  # index of the second "-"
            ...
            function = (
                function[:third_dash_index] + "," + function[third_dash_index + 1 :]
            )
            print(function)
